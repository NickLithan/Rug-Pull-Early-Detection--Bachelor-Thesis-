...
class ExtremeDrawdownNoPosteriorRecovery(TargetsConstructor):
    ...
    def get_targets_list(cls) -> list[str]:
        out = [f"md_price_no_post_rc_{i+1}" for i in range(len(cls._get_price_thresholds()))]
        out += [f"md_liq_no_post_rc_{i+1}" for i in range(len(cls._get_liq_thresholds()))]
        return out
    ...
    def calculate(cls, row) -> dict:
        ...
        out = {}
        for i, price_thresholds in enumerate(cls._get_price_thresholds()):
            ...
            if last_price is None or max_price is None:
                out[f"md_price_no_post_rc_{i+1}"] = np.nan
                ...
                out[f"md_price_no_post_rc_{i+1}"] = 0
            else:
        ...
        for i, liq_thresholds in enumerate(cls._get_liq_thresholds()):
            ...
            if last_tvl is None or max_tvl is None:
                ...
                out[f"md_liq_no_post_rc_{i+1}"] = int(MD_condition_liq and RC_condition_liq)
...
class ExtremeDrawdownTVL(TargetsConstructor):
    ...
    @classmethod
    def get_targets_list(cls) -> list[str]:
        return [f"md_liq_{i+1}" for i in range(len(cls._get_thresholds()))]
    ...
    def calculate(cls, row) -> dict:
        ...
        out = {}
        for i, drop_threshold_tvl in enumerate(cls._get_thresholds()):
            if max_tvl is None:
                out[f"md_liq_{i+1}"] = np.nan
            elif posterior_min_tvl is None or np.isclose(max_tvl, posterior_min_tvl) or np.isclose(max_tvl, 0):
                out[f"md_liq_{i+1}"] = 0
            else:
                MD_condition_liq = (max_tvl - posterior_min_tvl) >= (drop_threshold_tvl * max_tvl)
                out[f"md_liq_{i+1}"] = int(MD_condition_liq)
...
class ProlongedInactivity(TargetsConstructor):
    ...
    @classmethod
    def get_targets_list(cls) -> list[str]:
        return [f"idle_{i+1}" for i in range(len(cls._get_thresholds()))]

    @classmethod
    def calculate(cls, row) -> dict:
        longest_inactivity = row.get("longest_inactivity")
        return {
            f"idle_{i+1}": np.nan if longest_inactivity is None else int(longest_inactivity >= inactivity_threshold)
            for i, inactivity_threshold in enumerate(cls._get_thresholds())
        }
